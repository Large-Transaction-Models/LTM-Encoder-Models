...
class AaveBasicDataset(Dataset):
    def __init__(self,
                 cls_task,
                 user_ids=None,
                 seq_len=10,
        ...
        :param fname: file name of original data
        :param user_level_cached: if user level data will be used, if so, cached data will be loaded
        ...
        :param fextension: file name extension used by cached user level data and final vocab file
        ...
        :param long_and_sort: if the encoded data is very long and sorted by user ids and time
        :param resample_method: a method for data resampling. If None, then not resampling, if "upsample",
        then positive samples will be upsampled, if "downsample", then negative samples will be downsampled
        :param resample_ratio: ratio for resample data, resample_ratio = # of negative samples / # of positive samples
        :param resample_seed: random seed for resample
        """
        ...
        self.root = root
        self.fname = fname
        self.user_level_cached = user_level_cached 
        ...
        if self.vocab_cached and path.exists(self.external_vocab_path):
            log.info(f"using cached vocab from {self.external_vocab_path}")
            self.vocab = pickle.load(open(self.external_vocab_path, 'rb'))
            # Check and reorder columns in trans_table to match vocab
            vocab_fields = self.vocab.get_field_keys(remove_target=True, ignore_special=True)
            ignore_columns = ["rowNumber", "user", "timestamp", "id"]
            trans_columns = list(self.trans_table.columns)
            # log.debug(f"original columns in trans_table: {trans_columns}")
            # log.debug(f"vocab field ordering: {vocab_fields}")
            # Ensure all vocab fields are in trans_table, excluding ignored columns
            if not set(vocab_fields).issubset(set(trans_columns)):
                raise ValueError("Mismatch between vocab fields and trans_table columns.")

            # Reorder trans_table columns
            reordered_columns = ignore_columns + [col for col in vocab_fields if col in trans_columns]
            self.trans_table = self.trans_table[reordered_columns]
            # log.debug(f"new column ordering: {reordered_columns}")
    ...
    def resample_data(self):
        ...
        if self.resample_method == "downsample":
            ...
            keep_more_idx = rng.choice(more_idx, num_more_keep, replace=True) # !!!!!!!!!!not sure
