...
class AaveWithTimePosAndStaticSplitDataset(AaveBasicDataset):
    def __init__(self,
                 cls_task,
                 user_ids=None,
                 seq_len=10,
    ...
    def user_level_split_data(self):
        ...
        if self.user_level_cached and path.isfile(fname):
            ...
            other_columns = ['type','rownumber', 'user', 'liquidator', 'timestamp']
            not_use_columns = ['timeFeature']
            # we are using the user-level features as static columns
            static_columns = [col for col in self.trans_table.columns if col.startswith('user')]
            self.static_ncols = len(static_columns) + 1
