...
class BasicDataset(Dataset):
    ...
    def save_vocab(self):
        file_name = path.join(self.save_vocab_dir, f'vocab.nb')
        log.info(f"saving vocab at {file_name}")
        self.vocab.save_vocab(file_name)
        if not self.vocab_cached:
            self.vocab_path = path.join(self.save_vocab_dir, f'vocab_ob')
    ...
    def load_vocab(self):
        for vocab_prefix in ['dynamic', 'time_feature', 'static']:
            file_name = path.join(self.preload_vocab_dir, f'{vocab_prefix}_vocab_ob')
