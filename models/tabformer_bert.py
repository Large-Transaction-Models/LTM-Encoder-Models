...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    level=logging.INFO  # 或者根据需要设为 INFO, WARNING 等
)
...
class TabFormerBertForMaskedLM(BertForMaskedLM):
    ...
    def forward(
        ...
        for field_idx, field_name in enumerate(field_names):
            ...
            # logger.debug(f"Processing field: {field_name} with field_idx: {field_idx}")
    
            # logger.debug(f"Global IDs for field '{field_name}': {global_ids_field}")
    
            prediction_scores_field = prediction_scores[:, col_ids, :][:, :, global_ids_field]
            masked_lm_labels_field = masked_lm_labels[:, col_ids]       

            
            # logger.debug(f"Unique values in masked_lm_labels_field (used as global ids) for field {field_name}: {torch.unique(masked_lm_labels_field).tolist()}")
            
            masked_lm_labels_field_local = self.vocab.get_from_global_ids(global_ids=masked_lm_labels_field,
                                                                          what_to_get='local_ids')

            # logger.debug(f"After getting local ids from global ids: {torch.unique(masked_lm_labels_field_local).tolist()}")
            
            masked_lm_labels_field_local = masked_lm_labels_field_local - 1
            masked_lm_labels_field_local[masked_lm_labels_field_local == -101] = -100
    
            # logger.debug(f"Unique values in masked_lm_labels_field_local for field '{field_name}': {torch.unique(masked_lm_labels_field_local).tolist()}")
            # logger.debug(f"Length of global_ids_field for field '{field_name}': {len(global_ids_field)}")
            ...
            if (masked_lm_labels_field_local + 100).sum() != 0:
                ...
                # logger.debug(f"Shape of prediction_scores_field: {prediction_scores_field.shape}")
                # logger.debug(f"Shape of masked_lm_labels_field_local: {masked_lm_labels_field_local.shape}")
                # logger.debug(f"Field '{field_name}' has {nfeas} classes.")
